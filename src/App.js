...
function App() {
    ...
    const handleCreateDoc = async ()=>{
        ...
        //     love:'ha ji hyung'
        // })
        const createdDocId = await firebaseManager.createDocByPath('posts/lfj3n4JJg2QmvJLJlCgG/comments', {
            content:'두번쨰 덧글이야!'
        })
    ...
    const handleReadDocs = async ()=>{
       // const docs = await firebaseManager.readDocs('posts');
        const docs = await firebaseManager.readDocsByPath('posts/lfj3n4JJg2QmvJLJlCgG/comments');
        console.log(docs);
    ...
    const handleReadDoc = async ()=>{
        ...
        const doc = await firebaseManager.readDocByPath('posts/lfj3n4JJg2QmvJLJlCgG/comments/mFknQBVo44w45gqJsDV4');
        console.log(doc)
